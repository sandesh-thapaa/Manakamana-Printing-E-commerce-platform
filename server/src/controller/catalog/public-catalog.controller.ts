...
import { getVariantPricingCombination, calculateOrderAmount } from "../../services/catalog/product-pricing.service";
...
export const calculatePriceController = async (req: Request, res: Response) => {
  try {
    ...
    const pricingRow = await getVariantPricingCombination(variantId, options);
    if (!pricingRow) {
